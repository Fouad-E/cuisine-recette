...
class FavoritesRecipe extends Component {
  ...
  componentDidMount() {
    // Load the favorites recipes from localstorage and define these in state
  ...
  deleteRecipe(id) {
    // Delete a recipe from favorites recipes list after clicking on delete icon
        ...
        {favoritesRecipes.map((recipe) => (
          <Card className="catalogueRecipe">
            <Link
              ...
              onClick={() => this.handleSetIdCurrentRecipe(recipe.id)}
            >
              <CardImg top src={recipe.imageUrl} alt={recipe.id} />
            </Link>
            <CardBody>
